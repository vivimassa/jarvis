...
class _BrowserSession:
    ...
    # ── YouTube Music playback (uses the logged-in Chrome profile) ───────────
    async def _ytm_is_playing(self, page) -> bool:
        try:
    ...
    async def _ytm_start_playback(self, page, prefer_shuffle: bool) -> bool:
        """Try the most reliable play affordances on YouTube Music, in order.
        Playwright clicks count as trusted user gestures, so autoplay-with-sound
        is allowed."""
        attempts = []
        if prefer_shuffle:
            attempts.append(page.get_by_role("button", name=re.compile("shuffle", re.I)))
        attempts += [
            page.locator("ytmusic-play-button-renderer").first,
            page.get_by_role("button", name=re.compile(r"^\s*play", re.I)).first,
            page.locator("#play-button").first,
            page.locator("ytmusic-responsive-list-item-renderer").first,
        ]
        for loc in attempts:
            try:
                if await loc.count() == 0:
                ...
                if await self._ytm_is_playing(page):
                    return True
            except Exception:
                continue
        return await self._ytm_is_playing(page)

    async def play_music(self, query: str = "", shuffle: bool = True) -> str:
        page = await self._get_page()
        q = (query or "").strip()
        if q:
            ...
            # LM = the account's auto-generated "Liked Music" playlist (favourites)
            url  = "https://music.youtube.com/playlist?list=LM"
            what = "your liked music"

        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=30_000)
        except PlaywrightTimeout:
            pass
        await asyncio.sleep(2.5)   # let the single-page app render
        ...
        try:
            ...
            pass
        signed_out = "sign in" in body and "library" not in body

        played = await self._ytm_start_playback(page, prefer_shuffle=shuffle and not q)

        if played:
            return f"Playing {what} on YouTube Music."
        if signed_out:
            return ("Opened YouTube Music, but this Chrome profile isn't signed in "
                    "(it may already be open elsewhere, so JARVIS got a fresh profile). "
                    "Sign in once in this window and try again.")
        return (f"Opened YouTube Music for {what}, but couldn't confirm playback "
                "started — the page layout may have changed. Try saying 'play' again.")

    async def _ytm_page(self):
        """Find the YouTube Music tab in this browser (playback may be on a tab
        other than the active one), else fall back to the active page."""
        if self._context:
            for p in self._context.pages:
                try:
                    if "music.youtube.com" in (p.url or ""):
                        return p
                except Exception:
                    continue
        return await self._get_page()

    async def media_control(self, action: str, value=None) -> str:
        """Transport controls for the music playing in YouTube Music."""
        page = await self._ytm_page()
        a = (action or "").lower().strip()
        V = "const v=document.querySelector('video');"
        js = None
        if a in ("pause", "stop"):
            js = f"{V} if(v) v.pause(); 'ok'"
        elif a in ("play", "resume"):
            js = f"{V} if(v) v.play(); 'ok'"
        elif a in ("toggle", "playpause"):
            js = f"{V} if(v){{v.paused?v.play():v.pause();}} 'ok'"
        elif a in ("next", "skip"):
            js = "(document.querySelector('.next-button')||document.querySelector('tp-yt-paper-icon-button.next-button'))?.click(); 'ok'"
        elif a in ("prev", "previous", "back"):
            js = "(document.querySelector('.previous-button')||document.querySelector('tp-yt-paper-icon-button.previous-button'))?.click(); 'ok'"
        elif a in ("volume_up", "louder", "up"):
            js = f"{V} if(v) v.volume=Math.min(1, v.volume+0.1); 'ok'"
        elif a in ("volume_down", "quieter", "down"):
            js = f"{V} if(v) v.volume=Math.max(0, v.volume-0.1); 'ok'"
        elif a in ("set_volume", "volume"):
            try:
                n = float(value)
                vol = max(0.0, min(1.0, n / 100.0 if n > 1 else n))
            except Exception:
                vol = 0.5
            js = f"{V} if(v) v.volume={vol}; 'ok'"
        elif a == "mute":
            js = f"{V} if(v) v.muted=true; 'ok'"
        elif a == "unmute":
            js = f"{V} if(v) v.muted=false; 'ok'"
        else:
...
def browser_control(
    ...
    try:
        if action == "go_to":
            ...
            result = sess.run(
                sess.play_music(params.get("query", ""), params.get("shuffle", True)),
                timeout=120,
            )
        elif action == "media_control":
            result = sess.run(
                sess.media_control(params.get("media_action", ""), params.get("value")),
                timeout=30,
            )
